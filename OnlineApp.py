    ...
    if all_cases:
        ...
        with col_s:
            ...
            q_input = st.text_input(
                ...
                placeholder="請輸入關鍵字查詢，多個關鍵字請以空格格開", 
        ...
        if st.session_state.q_text:
            ...
            for c in all_cases:
                ...
                for t in terms:
                    ...
                    # 🔒 鎖死精度關鍵：如果是 3 碼英文，使用正則比對 (\b)
                    if len(t) == 3 and t.isalpha():
                        t_hit = any(re.search(rf'\b{re.escape(w)}\b', title.lower()) or w == cid for w in exp)
                        p_hit = any(re.search(rf'\b{re.escape(w)}\b', f_path.lower()) for w in exp)
                    else:
                        t_hit = any(w in title.lower() for w in exp) or any(w == cid for w in exp)
                    ...
                    with st.expander("查閱測試步驟", expanded=False):
                        ...
                        if isinstance(steps_data, list) and len(steps_data) > 0:
                            for s_idx, s in enumerate(steps_data, 1):
                                c_html = render_content(s.get('content', ''))
                                e_html = render_content(s.get('expected', ''))
                                st.markdown(f'''
