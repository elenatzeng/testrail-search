    ...
    if all_cases is not None:
        ...
        if final_query:
            ...
            for case_item in all_cases:
                ...
                if is_all_match:
                    # 🚀 這裡修正了 NameError：統一使用 case_item
                    author_name = "Esther" # 預設值，如果妳有正確的欄位可以改成 case_item.get('欄位')
                    ...
                    scored_results.append((total_score, case_item, u_info, author_name))
            ...
            for _, item, u_info, author_name in scored_results:
                cid = str(item.get('id'))
                # 🚀 亮燈與外框顏色判定
                tag_color = '#4CAF50' if u_info.get('is_active') else '#ff4b4b'
