...
with st.sidebar:
    ...
    if st.button("💾 儲存資訊至網址", use_container_width=True): # 標記 1
        st.query_params.update(url=tr_url, user=tr_user, pw=tr_pw, pid=project_id, sid=suite_id)
        st.success("✅ 已儲存")

    if st.button("🔄 強制更新數據", use_container_width=True): # 標記 2
        st.cache_data.clear()
        st.rerun()
...
if tr_url and tr_user and tr_pw:
    ...
    if all_cases:
        ...
        if final_query:
            ...
            # 🚀 修正點：自動過濾掉長度過短的符號 (如橫槓)，增加搜尋成功率
            raw_terms = [t for t in final_query.strip().split() if len(t) > 1 or t.isalnum()]
            scored_results = []
