...
st.set_page_config(page_title="TestRail AI Search", layout="wide", page_icon="🧪import streamlit as st
from style import apply_custom_style
from utils import clean_html, fetch_data_from_tr, multi_lang_search
from users import USER_CONFIG, DEFAULT_CONFIG
from keywords import SEARCH_DICTIONARY
...
if tr_url and tr_user and tr_pw:
    ...
    if all_cases is not None:
        ...
        # 🚀 解決「歪掉」：使用 vertical_alignment="bottom"
        ...
        if final_query:
            ...
            for c in all_cases:
                ...
                # 先解析內容再搜尋，避免搜到 HTML 標籤
                clean_c_body = clean_html(str(c.get('custom_steps','')) + str(c.get('custom_steps_separated',''))).lower()
                ...
                if is_all_match:
                    ...
                    if len(clean_c_body.strip()) < 10: total_score -= 500000 
