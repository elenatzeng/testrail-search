        ...
        if st.session_state.q_text:
            ...
            for _, item, u in results:
                ...
                with st.expander("查閱測試步驟", expanded=False):
                    ...
                    # 🔥 究極還原淨化器：把圖片變符號，文字原封不動
                    def ultimate_pure_render(text):
                        if not text: return ""
                        text = re.sub(img_pattern, ' [🖼️ 圖片附件] ', text).strip()
                        # 將文字中的 HTML 特殊字元轉義，防止亂碼
                        text = text.replace('<', '&lt;').replace('>', '&gt;')
                        return text

                    if isinstance(steps_data, list) and len(steps_data) > 0:
                        for s_idx, s in enumerate(steps_data, 1):
                            c_txt = ultimate_pure_render(s.get('content', ''))
                            e_txt = ultimate_pure_render(s.get('expected', ''))
                            ...
                            st.markdown(f'''
                                <div style="border-left: 4px solid #4CAF50 !important; padding-left: 20px; margin-left: 5px; margin-bottom: 25px;">
                                    ...
                                    <div style="background:#1c2128; border:1px solid #30363d; border-radius:12px; padding:15px 20px; color:#c9d1d9; font-size:14px; line-height:1.8; margin-bottom:15px; white-space:pre-wrap; font-family:inherit;">{c_txt if c_txt else "(無操作內容)"}</div>
                                    <div style="color:white; font-weight:bold; margin-bottom:8px; font-size:16px;">Expected:</div>
                                    <div style="background:#1c2128; border:1px solid #30363d; border-radius:12px; padding:15px 20px; color:#c9d1d9; font-size:14px; line-height:1.8; white-space:pre-wrap; font-family:inherit;">{e_txt if e_txt else "(無預期結果)"}</div>
                                </div>
