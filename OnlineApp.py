...
if tr_url and tr_user and tr_pw:
    ...
    if all_cases:
        ...
        q_input = st.text_input("● 搜尋內容:", value=st.session_state.q_text, placeholder="充值 CNY")
                ...
                with st.expander("🔽 查看測試步驟"):
                    ...
                    if isinstance(steps, list):
                        for i, s in enumerate(steps, 1):
                            st.markdown(f"""<div class="step-container"><b>Step {i}:</b><div class="step-content-box">{s.get('content','')}</div><b>Expected:</b><div class="step-content-box" style="border-left:1px dashed #444;">{s.get('expected','')}</div></div>""", unsafe_allow_html=True)
                    else:
