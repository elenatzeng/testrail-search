...
if tr_url and tr_user and tr_pw:
    ...
    if all_cases is None:
        st.error(f"❌ 無法取得數據。原因：{sync_time}")
        st.info("💡 請檢查側邊欄的 URL、Email、API Key 是否正確。")
    elif not all_cases:
        st.warning(f"⚠️ 已連線成功，但在 Suite #{sid} 中找不到任何測試案例。")
    else:
        # ✨ 數據正常，開始顯示搜尋框
            ...
            if not results:
                ...
                for _, path, item, u in results:
                    ...
                    with st.expander("查阅测试步骤", expanded=False):
                        ...
                        # 🎯 這裡直接用妳覺得「最漂亮」的渲染 logic
                        def final_render(text):
                            if not text: return "(无内容)"
                            # 先過濾 HTML 標籤
                            text = clean_html(text) # 只做基礎標籤清理
