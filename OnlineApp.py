    ...
    if all_cases:
        ...
        if st.session_state.q_text:
            ...
            for c in all_cases:
                ...
                for t in terms:
                    # 📖 呼叫字典聯想（如果字典沒寫 CNY，expanded 裡至少會有 ['cny']）
                    expanded = multi_lang_search(t, SEARCH_DICTIONARY)
                    
                    # 只要標題、路徑或 ID 包含這組詞中的任何一個，就算命中這組關鍵字
                    term_hit = any(w.lower() in title_low or w.lower() in path_low or w == cid for w in expanded)
                ...
                if is_match:
                    ...
                    results.append((10 + quality_weight, path_low, c, user_info))
            ...
            if not results:
                ...
                for _, path_name, item, u in results:
