        ...
        if st.session_state.q_text:
            ...
            for c in all_cases:
                ...
                for t in terms:
                    ...
                    term_hit = any(w.lower() in title_low or w.lower() in path_low or w.lower() == cid_str for w in exp)
                    if term_hit:
                        if any(w.lower() in title_low for w in exp): match_score += 10
                        elif any(w.lower() in path_low for w in exp): match_score += 1
                    else:
            ...
            if not results:
                ...
                for _, _, item, u in results:
                    ...
                    with st.expander("查阅测试步骤", expanded=False):
                        # 🎯 這裡直接交給妳最信任的 utils.py 處理，不做二次處理
                            ...
                            for s_idx, s in enumerate(steps_data, 1):
                                st.markdown(f'''
                                    <div style="border-left:4px solid #2ea44f; padding-left:20px; margin-left:5px; margin-bottom:30px; display:block;">
                                        ...
                                        <div class="content-box">{s["expected"]}</div>
                                    </div>
                                ''', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div class="content-box">{steps_data}</div>', unsafe_allow_html=True)
                    st.markdown("---")
else:
    st.info("👈 请先在左侧完成连线设定。")
