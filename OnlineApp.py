...
if "show_sidebar" not in st.session_state:
    st.session_state.show_sidebar = True  # 預設開啟
...
if st.session_state.show_sidebar:
    with st.sidebar:
        ...
        st.markdown("---")
        if st.button("⬅️ 收起設定欄", use_container_width=True):
            st.session_state.show_sidebar = False
            st.rerun()
else:
    # 當側邊欄隱藏時，在主頁面左上角顯示一個浮動的開啟按鈕
    st.markdown("""
        <style>
        .floating-open-btn {
            position: fixed; top: 15px; left: 15px; z-index: 999999;
            background-color: #32CD32 !important; color: black !important;
            padding: 8px 15px; border-radius: 8px; font-weight: bold;
            cursor: pointer; border: none; box-shadow: 0 0 10px rgba(50, 205, 50, 0.5);
        }
        </style>
    """, unsafe_allow_html=True)
    if st.button("➡️ 打開連線設定", key="open_sidebar_btn"):
