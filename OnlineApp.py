...
if tr_url and tr_user and tr_pw:
    ...
    if all_cases:
        ...
        if st.session_state.q_text:
            ...
            for c in all_cases:
                cid, title = str(c.get('id')), str(c.get('title', ''))
            ...
            for _, path, item, u in results:
                ...
                c1.markdown(f'<div style="display:flex; align-items:center; margin-bottom:5px;"><span style="font-size:18px; font-weight:bold;">{item.get("title")} (#{cid_str})</span>{tag}</div>', unsafe_allow_html=True)
