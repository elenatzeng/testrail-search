    ...
    if all_cases is not None:
        ...
        if final_query:
            ...
            for _, item, u_info in scored_results:
                ...
                # ✨ 🚀 終極修正：字體放大到 13px，加粗，完美膠囊框
                author_tag_html = f"""
                    <span class="author-tag" style="border-color: {author_color} !important; box-shadow: 0 0 5px {author_color} !important; border-radius: 25px !important; border: 2px solid !important;">
                        <span style="font-size: 15px; margin-right: 5px;">{status_emoji}</span>
