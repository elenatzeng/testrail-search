            ...
            for _, item, u in results:
                ...
                with st.expander("查閱測試步驟", expanded=False):
                    ...
                    if isinstance(steps, list):
                        ...
                        if not has_visible:
                            st.markdown('<div style="color:#666; font-size:14px; margin-left:15px;">(僅包含圖片附件)</div>', unsafe_allow_html=True)
                    
                    elif steps:
                        clean_steps = re.sub(img_pattern, '', steps).strip()
                        if clean_steps:
                            st.markdown(f'<div class="step-container"><div class="step-box">{clean_steps}</div></div>', unsafe_allow_html=True)
                        else:
                            st.markdown('<div style="color:#666; font-size:14px; margin-left:15px;">(僅包含圖片附件)</div>', unsafe_allow_html=True)
                st.markdown("---")
